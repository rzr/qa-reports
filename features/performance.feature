...
Feature: Loading times
  ...
  Scenario: Upload bigger reports via API and compare them and load history
    ...
    Then the time spent for the "import 2 reports via API" step should be less than 50 seconds
    ...
    And the client sends file "features/resources/2000_cases.csv" via the REST API
    And the client sends file "features/resources/2000_cases.csv" via the REST API
    Then the time spent for the "import 4 more big reports via API" step should be less than 40 seconds
